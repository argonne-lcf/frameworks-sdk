...
CONDA_ENV_NAME=dpnp_0.19.1_dpctl_0.21.1_scikit_2025.10.1_pytorch_2.10.0_rel_oneapi_2025.3.1_numpy_2.2.6_python_3.12.12
...
conda create python=3.12.12 icu=73 glog gflags fmt bazel \
...
## dpnp/0.19.1 specific
unset CXX

python setup.py clean --all

CXX=$(which dpcpp) python setup.py bdist_wheel --dist-dir ${TMP_WORK}/${CONDA_ENV_NAME} -- -G Ninja -DCMAKE_C_COMPILER:PATH=$(which icx) -DCMAKE_CXX_COMPILER:PATH=$(which icpx) \
    2>&1 | tee ${TMP_WORK}/${CONDA_ENV_NAME}/"dpnp-build-whl-$(tstamp).log"

#echo "Finished building dpnp/0.19.1, ipex/2.10.10+xpu triton_xpu_3.6.0 for PyTorch 2.9.1 wheel with numpy 2.2.6 with oneapi/2025.3.1"
