...
CONDA_ENV_NAME=jax_0.5.0_no_deps_conda_vllm_0.15.0_nre_ipex_2.10.10_pytorch_2.10.0_rel_oneapi_2025.3.1_numpy_2.2.6_python_3.12.8_RC3
...
TMP_WORK=/lus/tegu/projects/datasets/software/26.26.0/wheelforge/repositories/intel_openxla_02_04_2026
...
#git clone git@github.com:intel/intel-extension-for-openxla.git
cd intel-extension-for-openxla/xla/tools/pip_package
#git checkout 0.7.0
...
## Install JAX==0.5.0 requirements
# The Intel-OpenXLA wants scipy < 1.12.0 -- vLLM may not like it!
pip install --no-cache-dir -r ${WHEELHOUSE_TMP}/jax_0.5.0_xpu_separate_requirements.txt

pip install --no-deps --no-cache-dir --force-reinstall --no-build-isolation ${WHEELHOUSE_DOWNLOAD}/jax-*.whl 2>&1 | tee ${TMP_WORK}/${CONDA_ENV_NAME}/"jax-install-$(tstamp).log"
...
python xla_setup.py clean --all
python xla_setup.py bdist_wheel --dist-dir ${TMP_WORK}/${CONDA_ENV_NAME} 2>&1 | tee ${TMP_WORK}/${CONDA_ENV_NAME}/"intel-openxal-build-whl-$(tstamp).log"
...
pip install --no-deps --no-cache-dir --force-reinstall --no-build-isolation ${LOCAL_WHEEL_LOC}/intel_extension_for_openxla-*.whl 2>&1 | tee ${TMP_WORK}/${CONDA_ENV_NAME}/"opexla-install-$(tstamp).log"
