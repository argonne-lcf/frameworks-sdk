...
CONDA_ENV_NAME=debug_mpi4py_4.1.1_python_3.12.12
...
TMP_WORK=/lus/tegu/projects/datasets/software/26.26.0/wheelforge/repositories/mpi4py_4.1.1_01_29_2026
...
## Install scikit-2025.10.1
pip install --no-deps --no-cache-dir --no-build-isolation ${WHEELHOUSE_TMP}/scikit_learn_intelex-*.whl


export MPI4PY_BUILD_MPICC=$(which mpicc)
export MPI4PY_BUILD_MPILD=$(which mpicc)

#python setup.py clean --all
#python setup.py bdist_wheel --dist-dir ${TMP_WORK}/${CONDA_ENV_NAME} 2>&1 | tee ${TMP_WORK}/${CONDA_ENV_NAME}/"dpctl-build-whl-$(tstamp).log"
pip wheel --no-deps --no-build-isolation --wheel-dir=${TMP_WORK}/${CONDA_ENV_NAME} . 2>&1 | tee ${TMP_WORK}/${CONDA_ENV_NAME}/"mpi4py-build-whl-$(tstamp).log"
#echo "Finished building mpi4py/4.1.1, ipex/2.10.10+xpu triton_xpu_3.6.0 for PyTorch 2.9.1 wheel with numpy 2.2.6 with oneapi/2025.3.1"

LOCAL_WHEEL_LOC=${TMP_WORK}/${CONDA_ENV_NAME}
pip install --no-deps --no-cache-dir --no-build-isolation ${LOCAL_WHEEL_LOC}/mpi4py-*.whl 2>&1 | tee ${TMP_WORK}/${CONDA_ENV_NAME}/"mpi4py-install-$(tstamp).log"
echo "Finished installing mpi4py/4.1.1 the wheel and dependencies"
