...
CONDA_ENV_NAME=torchao_0.15.0_nre_ipex_2.10.10_pytorch_2.10.0_rel_oneapi_2025.3.1_numpy_2.2.6_python_3.12.8
...
TMP_WORK=/lus/tegu/projects/datasets/software/26.26.0/wheelforge/repositories/torchao_0.15.0_01_29_2026
cd $TMP_WORK
...
#git clone git@github.com:pytorch/ao.git
cd ao
...
MAX_JOBS=16 python setup.py bdist_wheel --dist-dir ${TMP_WORK}/${CONDA_ENV_NAME} 2>&1 | tee ${TMP_WORK}/${CONDA_ENV_NAME}/"torchao-build-whl-$(tstamp).log"
echo "Finished building torchao/0.15.0, ipex/2.10.10+xpu triton_xpu_3.6.0 for PyTorch 2.9.1 wheel with numpy 2.2.6 with oneapi/2025.3.1"

LOCAL_WHEEL_LOC=${TMP_WORK}/${CONDA_ENV_NAME}
pip install --no-deps --no-cache-dir --force-reinstall $LOCAL_WHEEL_LOC/torchao-*.whl 2>&1 | tee ${TMP_WORK}/${CONDA_ENV_NAME}/"torchao-install-$(tstamp).log"
