...
CONDA_ENV_NAME=torchcomms_0.1.0_deepspeed_0.18.5_no_deps_conda_vllm_0.15.0_nre_ipex_2.10.10_pytorch_2.10.0_rel_oneapi_2025.3.1_numpy_2.2.6_python_3.12.8
...
conda create python=3.12.8 icu=73 glog gflags --prefix ${ENVPREFIX} --override-channels \
...
pip install --no-deps --no-cache-dir --no-build-isolation deepspeed==0.18.5 2>&1 | tee ${TMP_WORK}/${CONDA_ENV_NAME}/"deepspeed-install-$(tstamp).log"
