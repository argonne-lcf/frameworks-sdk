...
CONDA_ENV_NAME=dpctl_gitb24f931_icu73_vllm_0.25.1_triton_3.7.2_nre_pt_2.13.0_rel_one_2026.1.0_np_2.3.5_python_3.12.12
...
## CHANGE HERE!!!
TMP_WORK=/lus/tegu/projects/datasets/software/26.181.0/wheelforge/repositories/dpctl_b24f931_07_29_2026
cd $TMP_WORK
...
python setup.py bdist_wheel --dist-dir ${TMP_WORK}/${CONDA_ENV_NAME} -- \
    -G Ninja -DCMAKE_C_COMPILER:PATH=$(which icx) -DCMAKE_CXX_COMPILER:PATH=$(which icpx) \
    2>&1 | tee ${TMP_WORK}/${CONDA_ENV_NAME}/"dpctl-build-whl-$(tstamp).log"
echo "Finished building dpctl+gitb24f931, triton_xpu_3.7.2 for PyTorch 2.13.0 wheel with numpy 2.3.5 with oneapi/2026.1.0"

LOCAL_WHEEL_LOC=${TMP_WORK}/${CONDA_ENV_NAME}
pip install --no-deps --no-cache-dir --no-build-isolation ${LOCAL_WHEEL_LOC}/dpctl-*.whl 2>&1 | tee ${TMP_WORK}/${CONDA_ENV_NAME}/"dpctl-install-$(tstamp).log"
