...
CONDA_ENV_NAME=scikitlearnintelex_2026.1.0_vllm_0.25.1_triton_3.7.2_nre_pt_2.13.0_rel_one_2026.1.0_np_2.3.5_python_3.12.12
...
conda create python=3.12.12 glog=0.4.0 gflags=2.2.2 fmt --prefix ${ENVPREFIX} --override-channels \
           --channel conda-forge \
...
## CHANGE HERE!!!
TMP_WORK=/lus/tegu/projects/datasets/software/26.181.0/wheelforge/repositories/scikitlearnintelex_2026.1.0_07_28_2026
cd $TMP_WORK
...
python setup.py bdist_wheel --dist-dir ${TMP_WORK}/${CONDA_ENV_NAME} 2>&1 | tee ${TMP_WORK}/${CONDA_ENV_NAME}/"scikitlearnintelex-build-whl-$(tstamp).log"
echo "Finished building scikit_learn_intelex/2025.10.1, triton_xpu_3.7.2 for PyTorch 2.13.0 wheel with numpy 2.3.5 with oneapi/2026.1.0"

LOCAL_WHEEL_LOC=${TMP_WORK}/${CONDA_ENV_NAME}
pip install --no-deps --no-cache-dir --no-build-isolation ${LOCAL_WHEEL_LOC}/scikit_learn_intelex-*.whl 2>&1 | tee ${TMP_WORK}/${CONDA_ENV_NAME}/"scikitlearnintelex-install-$(tstamp).log"
echo "Finished installing scikit_learn_intelex/2026.1.0 the wheel and dependencies"
