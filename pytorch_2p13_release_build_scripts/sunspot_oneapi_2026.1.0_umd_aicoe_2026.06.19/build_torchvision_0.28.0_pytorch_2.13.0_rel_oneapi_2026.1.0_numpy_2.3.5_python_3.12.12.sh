...
CONDA_ENV_NAME=torchvision_0.28.0_nre_pt_2.13.0_rel_one_2026.1.0_pti_0.17.0_np_2.3.5_python_3.12.12
...
## CHANGE HERE!!!
TMP_WORK=/lus/tegu/projects/datasets/software/26.181.0/wheelforge/repositories/torchvision_0.28.0_07_21_2026
cd $TMP_WORK
...
pip install --no-cache-dir -r ${WHEELHOUSE_TMP}/torchvision_0.28.0_torch_2.13.0_triton_xpu_3.7.2_combined_requirements.txt
...
export PYTORCH_VERSION='2.13.0a0+gitcf30153'
...
MAX_JOBS=16 python setup.py bdist_wheel --dist-dir ${TMP_WORK}/${CONDA_ENV_NAME} 2>&1 | tee ${TMP_WORK}/${CONDA_ENV_NAME}/"torchvision-build-whl-$(tstamp).log"
echo "Finished building torchvision/0.28.0, ipex/2.10.10+xpu triton_xpu_3.7.2 for PyTorch 2.13.0 wheel with numpy 2.3.5 with oneapi/2026.1.0"

LOCAL_WHEEL_LOC=${TMP_WORK}/${CONDA_ENV_NAME}
pip install --no-deps --no-cache-dir --force-reinstall $LOCAL_WHEEL_LOC/torchvision-*.whl 2>&1 | tee ${TMP_WORK}/${CONDA_ENV_NAME}/"torchvision-install-$(tstamp).log"
