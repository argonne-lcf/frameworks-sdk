...
CONDA_ENV_NAME=RC3_vllm_0.25.1_triton_3.7.2_torchcomms_0.3.1-rc1_nre_pt_2.13.0_rel_one_2026.1.0_np_2.3.5_python_3.12.12
...
## CHANGE HERE!!!
TMP_WORK=/lus/tegu/projects/datasets/software/26.181.0/wheelforge/repositories/vllm_0.25.1_07_22_2026
cd $TMP_WORK
...
rm -rf ${TMP_WORK}/vllm/build
rm -rf ${TMP_WORK}/vllm/dist
rm -rf ${TMP_WORK}/vllm/*.egg-info
echo "Removing done"
...
CMAKE_ARGS="-DCMAKE_CXX_COMPILER=$(which g++) -DCMAKE_C_COMPILER=$(which gcc)" \
    ...
    ${TMP_WORK}/vllm --wheel-dir ${LOCAL_WHEEL_LOC} \
    2>&1 | tee ${LOCAL_WHEEL_LOC}/"vllm-build-whl-$(tstamp).log"
echo "Finished building vllm/0.25.1, triton_xpu_3.7.1 for PyTorch 2.13.0 wheel with numpy 2.3.5 with oneapi/2026.1.0"

pip install --no-deps --no-cache-dir --force-reinstall ${LOCAL_WHEEL_LOC}/vllm-*.whl 2>&1 | tee ${LOCAL_WHEEL_LOC}/"vllm-install-$(tstamp).log"
