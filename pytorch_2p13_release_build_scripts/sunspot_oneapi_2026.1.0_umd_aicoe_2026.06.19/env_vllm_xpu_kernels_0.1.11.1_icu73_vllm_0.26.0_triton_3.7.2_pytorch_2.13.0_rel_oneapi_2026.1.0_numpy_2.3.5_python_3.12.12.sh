...
CONDA_ENV_NAME=RC4_vllm_xpu_kernels_0.1.11.1_icu73_vllm_0.26.0_triton_3.7.2_nre_pt_2.13.0_rel_one_2026.1.0_np_2.3.5_python_3.12.12
...
pip install --no-deps --no-cache-dir --force-reinstall ${WHEELHOUSE_TMP}/torchcomms-*.whl
echo "Finished installing the torch-2.13.0 ecosystem  with numpy/2.3.5 and dependencies"
...
pip install --no-deps "auto_round_lib==0.14.1" ## https://github.com/vllm-project/vllm/blob/v0.26.0/requirements/xpu.txt
pip install --no-cache-dir -r ${WHEELHOUSE_TMP}/vllm_0.26.0_xpu_separate_requirements.txt
...
pip install --no-deps --no-cache-dir --force-reinstall ${WHEELHOUSE_DOWNLOAD}/triton_xpu-*.whl
pip install --no-deps --no-cache-dir --force-reinstall ${WHEELHOUSE_TMP}/vllm_xpu_kernels-*.whl
pip install --no-deps --no-cache-dir --force-reinstall ${WHEELHOUSE_TMP}/vllm-*.whl
echo "Finished installing the triton-xpu/3.7.2, vllm-xpu-kernels/0.1.12, and vllm/0.26.0  with numpy/2.3.5 and dependencies"

##############################################################################

pip install --no-cache-dir -r ${WHEELHOUSE_TMP}/deepspeed_0.19.3_xpu_separate_requirements.txt
...
mkdir -p ${LOCAL_WHEEL_LOC}

## Install deepspeed==0.19.3
pip install --no-deps --no-cache-dir --no-build-isolation deepspeed==0.19.3
echo "Finished installing the deepspeed/0.19.3  with numpy/2.3.5 and dependencies"
pip install --no-deps --no-cache-dir --force-reinstall ${WHEELHOUSE_TMP}/scikit_learn_intelex-*.whl
echo "Finished installing the scikitlearnintelex/2026.1.0  with numpy/2.3.5 and dependencies"
##############################################################################

## dpctl requirements
pip install --no-cache-dir -r ${WHEELHOUSE_TMP}/dpctl_b24f931_xpu_separate_requirements.txt
...
pip uninstall -y numpy numpy-base
pip install --no-cache-dir numpy==2.3.5
pip install --no-deps --no-cache-dir --force-reinstall ${WHEELHOUSE_TMP}/dpctl-*.whl
echo "Finished installing dpctl+gitb24f931 the wheel and dependencies"
pip install --no-deps --no-cache-dir --force-reinstall ${WHEELHOUSE_TMP}/dpnp-*.whl
echo "Finished installing dpnp+git987f299 the wheel and dependencies"
##############################################################################
