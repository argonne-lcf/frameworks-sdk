...
skip reason) inside this repository; this script adds a `skip_reason`
to each of those tests in a cloned `suite.json` manifest so the runner
reports them as skipped instead of failed. Remove an entry from the
known-failures file once the underlying issue is fixed to re-enable the
test.
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Add skip_reason to known-failing tests in a suite.json manifest."
    )
    parser.add_argument(
        "--manifest",
        default="suite.json",
    ...
    print(f"{manifest_path}: marked {patched} test(s) as skipped per {failures_path}")
    return 0
